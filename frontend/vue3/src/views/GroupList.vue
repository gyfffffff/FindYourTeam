    ...
    <div style="margin: 10px 10px; height: 630px; background-color: white">
      ...
      <div style="padding: 20px">
        <el-table :data="tableData" border stripe style="width: 100%">
          ...
          <el-table-column prop="resp" label="角色" :filters="[
            { text: '负责人', value: '负责人' },
            { text: '成员', value: '成员' },
          ...
          <el-table-column label="操作">
            <template v-slot="scope">
              ...
              <el-button @click="gotoChat(scope.row)" type="success" size="small"
                style="margin-left: 25px">团队群聊</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!--        分页-->
      ...
      <!--      新增弹窗-->
      <el-dialog title="新增团队" v-model:visible="dialogVisible" width="30%" :before-close="handleClose">
      ...
      request
        ...
        .then((res) => {
          ...
          this.tableData.filter((item) => {
            if (item.resp === 0) item.resp = '成员'
            if (item.resp === 1) item.resp = '负责人'
          })
        })
    },
    add() {
