    ...
    <div style="margin: 10px 10px; height: 630px; background-color: white">
      ...
      <div style="padding: 20px">
        <el-table class="list" :data="tableData" border stripe style="width: 100%" @row-click="openDetails">
          <el-table-column prop="title" label="名称" width="220">
          </el-table-column>
          <el-table-column prop="startdate" label="创建时间" sortable width="220">
          </el-table-column>
          <el-table-column prop="ddl" label="截止时间" sortable width="220">
          </el-table-column>
          <el-table-column prop="done" label="状态" width="220" :filters="[
            { text: '已完成', value: '已完成' },
            { text: '未完成', value: '未完成' },
            { text: '待审核', value: '待审核' },
            { text: '已终止', value: '已终止' },
          ...
          <el-table-column prop="coop" label="任务类型" width="220" :filters="[
            { text: '项目任务', value: '项目任务' },
            { text: '个人任务', value: '个人任务' },
          ]" :filter-method="filterCoop">
          </el-table-column>
          <el-table-column prop="emer" label="重要程度" :filters="[
        ...
        <el-pagination @current-change="handleCurrentChange" v-model:current-page="currentPage" layout="prev, pager, next"
          :total="100" style="margin-top: 20px; margin-left: 530px; float: left">
        </el-pagination>
      ...
      <!--      新增弹窗-->
      <el-dialog title="新增任务" v-model:visible="dialogVisible" width="30%" :before-close="handleClose">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        ddl: [{ required: true, message: '请选择日期', trigger: 'blur' }],
        emer: [{ required: true, message: '请选择重要程度', trigger: 'blur' }],
      },
    }
  },
  created() {
