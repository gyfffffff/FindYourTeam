  ...
  <div style="background-color: #fff;">
    ...
    <el-row>
      <el-col :span="8">
        ...
        <div class="info">
          <div class="avatarBox">
            <el-avatar :size="100" class="avatar"> <span class="nameavatar">{{ name }}</span> </el-avatar>
          </div>
              ...
              <el-col :span="8" v-for="item in buttonArr" :key="item">
                <el-button
                  ...
                  :class="{
                    ...
                    activeButton: item.name === activeButton,
                  }"
                  :dark="isDark"
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      const uid = sessionStorage.getItem('stuid')
      http
        .get('/user/bystuid?stuid=' + uid)
        .then((res) => {
          console.log(54, res)
          this.infoArr = [
            { label: '姓名', value: res.data.data.name },
            { label: '学号', value: res.data.data.stuid },
          ]
        })
        .catch((err) => {
          console.log(err)
    ...
    fetchAvatar() {
      const url = '/user/getavatar?stuid=' + sessionStorage.getItem('stuid')
