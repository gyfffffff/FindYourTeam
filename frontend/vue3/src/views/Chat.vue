      ...
      <el-row>
        ...
        <el-col :span="16">
          ...
          >
            ...
            <div
              style="height: 360px; overflow: auto; border-top: 1px solid #ccc"
              v-html="content"
            ></div>
...
export default {
  ...
  methods: {
    ...
    init() {
      http.get('/group/bykey?key=' + this.groupKey).then((res) => {
        ...
        if (typeof WebSocket == 'undefined') {
          ...
          let socketUrl =
            'ws://localhost:9090/chatserver/' + name + '/' + _this.groupData.gid
    ...
    createContent(remoteuid, nowuid, text) {
      // 这个方法是用来将 json的聊天消息数据转换成 html的。
      let html
      // 当前用户消息
      if (nowuid) {
        // nowuid 表示是否显示当前用户发送的聊天消息，绿色气泡
        html =
