<template>
  <div class="main">
    ...
    <el-dialog title="新增项目" v-model="dialogFormVisible" :before-close="handleClose" destroy-on-close>
    ...
    <div style="width: 100%; padding: 20px; padding-top: 70px; border: 2px solid #dfdfdf; border-radius: 12px;">
      <el-table :data="tableData" border stripe style="width: 100%" @row-click="openDetails">
        ...
        <el-table-column prop="done" label="状态" :filters="[
          { text: '已完成', value: '已完成' },
          { text: '未完成', value: '未完成' },
          { text: '已终止', value: '已终止' },
...
<script>
import request from '@/http' // 引入新创建的http实例
export default {
  name: 'Myproj',
  data() {
    return {
      ...
      rules: {
        ...
        intro: [{ max: 800, message: '不能超过500字', trigger: 'blur' }],
        gid: [{ required: true, message: '请输入团队ID', trigger: 'blur' }],
  ...
  methods: {
    ...
    load() {
      request
        .get('/project/load', {
          params: { pageNum: this.currentPage, pageSize: 8, stuid: this.stuid },
        })
        .then((res) => {
