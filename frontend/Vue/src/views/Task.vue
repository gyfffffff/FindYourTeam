  ...
  <div>
    ...
    <div style="float: right; height: 627px; width: 760px; margin-right: 10px; background-color: white">
      ...
      <div v-else>
        ...
        <div v-if="this.taskData.coop === 1 && this.groupData.resp === 1" style="margin-top: 30px; text-align: center;">项目负责人操作： <el-button @click="approve" size="medium" type="success">审批任务</el-button></div>
      </div>
      <!--      审批弹窗-->
      <el-dialog
          title="审批任务"
    ...
    getTask(){
      http.get("/task/byid?tid="+this.taskId).then(res => {
        ...
        http.get("/project/byid?pid="+ this.taskData.pid).then(res => {
          ...
          http.get("/group/bypid?pid="+this.taskData.pid+"&uid="+this.uid).then(res => {
